...
class _EmergencyModeScreenState extends State<EmergencyModeScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: isDark ? const Color(0xFF140C0D) : const Color(0xFFFFF7F7),
      appBar: AppBar(
        backgroundColor: isDark ? const Color(0xFF1F1214) : const Color(0xFFFFECEC),
        title: const Row(
          children: [
            Icon(Icons.warning_amber_rounded, color: AppColors.emergencyRed, size: 26),
            SizedBox(width: 8),
            Text(
              "EMERGENCY CLARITY",
              style: TextStyle(
        ...
        actions: [
          Container(
            ...
            child: const Row(
              children: [
                Icon(Icons.wifi_off_rounded, size: 14, color: AppColors.protectiveGreen),
                SizedBox(width: 4),
                Text(
                  "Offline Ready",
                  style: TextStyle(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            ...
            // Big Emergency Activation CTA Button
            Obx(() {
                        ...
                        children: [
                          ...
                          Expanded(
                            child: Column(
                              ...
                              children: [
                                Text(
                                  isActive ? "EMERGENCY MODE ACTIVE" : "ACTIVATE EMERGENCY CLARITY",
            ...
            // Card 1: Estimated Cost Range
            _buildCostEstimateCard(isDark),
            const SizedBox(height: 16),

            // Card 2: Covered vs Out-of-Pocket Breakdown
            _buildCoverageBreakdownCard(isDark, controller),
            const SizedBox(height: 16),

            // Card 3: One-Tap Urgent Action Buttons
            _buildUrgentActionRow(context, controller, isDark),
            const SizedBox(height: 22),

            // Card 4: Nearby Network Hospitals with Real-time Estimates
            Row(
              ...
              children: [
                ...
                const StatusBadge(label: "Cashless Ready", type: BadgeType.cashless),
              ],
            ),
        ...
        children: [
          Row(
            ...
            children: [
              Row(
                children: [
                  Container(
                    ...
                    child: const Icon(Icons.attach_money_rounded, color: AppColors.accentGold, size: 20),
                  ),
              ...
              const StatusBadge(label: "AI Calculated", type: BadgeType.goldTag),
            ],
          ),
  ...
  Widget _buildUrgentActionRow(BuildContext context, AppController controller, bool isDark) {
    return Column(
      children: [
        CustomButton(
          text: "Share Emergency Pack with Family",
                ...
                children: [
                  Row(
                    ...
                    children: [
                      ...
                      StatusBadge(
                        label: hosp.isNetwork ? "Network Cashless" : "Non-Network",
                        type: hosp.isNetwork ? BadgeType.cashless : BadgeType.pending,
                      ),
                    ],
                  ),
                  const SizedBox(height: 8),
                  Row(
                    children: [
                      const Icon(Icons.near_me_rounded, size: 14, color: AppColors.primaryTeal),
                      ...
                      const SizedBox(width: 12),
                      const Icon(Icons.king_bed_rounded, size: 14, color: AppColors.protectiveGreen),
                      const SizedBox(width: 4),
                      Text(
                        hosp.bedAvailability,
                        style: const TextStyle(fontSize: 12.5, color: AppColors.protectiveGreen, fontWeight: FontWeight.w600),
                      ),
                    ],
                  ),
                  const SizedBox(height: 10),
                  Row(
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        ...
                        child: const Text("Navigate & Call Desk", style: TextStyle(fontSize: 12)),
                      ),
                    ],
                  ),
                ],
      ...
      Container(
        ...
        child: Column(
          ...
          children: [
            const Row(
              children: [
                Icon(Icons.family_restroom_rounded, color: AppColors.primaryTeal, size: 26),
                SizedBox(width: 10),
                Text(
            ...
            const SizedBox(height: 16),
            ...controller.familyMembers.map((member) => Padding(
                  padding: const EdgeInsets.only(bottom: 8.0),
                  child: Row(
                    mainAxisAlignment: MainAxisAlignment.spaceBetween,
                    children: [
                      Text("${member.name} (${member.relation})", style: const TextStyle(fontWeight: FontWeight.w600)),
                      Text(member.bloodGroup, style: const TextStyle(color: AppColors.emergencyRed, fontWeight: FontWeight.bold)),
                    ],
                  ),
                )),
            const SizedBox(height: 18),
            CustomButton(
              ...
              onPressed: () {
                Get.back();
                Get.snackbar("Emergency Pack Shared", "Family notified with policy details", backgroundColor: AppColors.protectiveGreen, colorText: Colors.white);
              },
            ),
          ],
        ),
  ...
  void _showCallAmbulanceModal(BuildContext context) {
    Get.dialog(
      AlertDialog(
        ...
        content: const Text("Dialing 108 Emergency Medical Services & Fortis Ambulance Dispatcher."),
        actions: [
          ...
          ElevatedButton(
            ...
            onPressed: () {
              Get.back();
              Get.snackbar("Calling 108", "Connecting to National Emergency Ambulance", backgroundColor: AppColors.emergencyRed, colorText: Colors.white);
            },
          ...
          children: [
            const Row(
              children: [
                ...
                SizedBox(width: 10),
                Text("Cashless Pre-Auth Sheet", style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold)),
              ],
            ),
            const SizedBox(height: 12),
            const Text("On-Device AI pre-filled document ready to show to hospital TPA desk."),
            const SizedBox(height: 14),
            Container(
              padding: const EdgeInsets.all(12),
              decoration: BoxDecoration(
              ...
              child: const Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  Text("• Policy: HDFC ERGO #994821", style: TextStyle(fontWeight: FontWeight.bold)),
